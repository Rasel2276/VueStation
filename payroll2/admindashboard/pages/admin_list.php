...
$query = "SELECT * FROM users WHERE role = 'admin' AND status != 'pending' ORDER BY created_at DESC";
...
<style>
    ...
    /* Dropdown CSS - Sob somoy niche khulbe */
    ...
    .dropdown-content { 
        ...
        position: absolute; 
        right: 0; 
        top: 100%; /* Sesh row hok ba prothom, nichei khulbe */
    ...
    .status-badge-active { background: rgba(75, 181, 67, 0.1); color: #4BB543; padding: 4px 8px; border-radius: 4px; font-size: 11px; }
    .status-badge-rejected { background: rgba(255, 77, 79, 0.1); color: #ff4d4f; padding: 4px 8px; border-radius: 4px; font-size: 11px; }

    /* --- MOBILE OPTIMIZATION (EXACT LEFT-RIGHT FIT) --- */
    @media (max-width: 768px) {
        ...
        .approval-table td { 
            text-align: right; 
            padding: 15px 15px; /* Soman padding left/right */
            position: relative; 
        ...
        .approval-table td::before { 
            ...
            position: absolute; 
            left: 15px; /* Label-er jonno perfect faka jayga */
            font-weight: 700; 
            color: #4BB543; 
...
<main class="main-content">
    <div class="main-panel">
        <div class="content-wrapper">
            <h3 style="margin-bottom: 10px; color: #fff; font-weight: 600; padding-left: 5px;">Admin Management List</h3>
                            ...
                            <?php if(mysqli_num_rows($result) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td data-label="Status">
                                        ...
                                        <?php else: ?>
                                            <span class="status-badge-rejected">Deactive</span>
                                        <?php endif; ?>
                                    </td>
                                    <td data-label="Action" style="overflow: visible;">
                                        <div class="action-dropdown">
                                            ...
                                            <div class="dropdown-content">
                                                <?php if($row['status'] == 'active'): ?>
                                                    <a href="admin_list.php?action=make_deactive&id=<?php echo $row['id']; ?>" onclick="return confirm('Deactivate this admin?')">Deactive</a>
                                                <?php else: ?>
                                                    <a href="admin_list.php?action=make_active&id=<?php echo $row['id']; ?>" onclick="return confirm('Activate this admin?')">Active</a>
                                                <?php endif; ?>
                                                <a href="admin_list.php?action=delete&id=<?php echo $row['id']; ?>" style="color: #ff4d4f;" onclick="return confirm('Are you sure you want to delete?')">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="5" style="text-align: center; padding: 30px;">No history found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
