          ...
          <table class="custom-category-table">
            ...
            <tbody>
              <tr v-for="sub in filteredSubcategories" :key="sub.id">
                ...
                <td data-label="Parent Category">{{ sub.category?.category_name || 'N/A' }}</td>
                <td data-label="Subcategory Name" style="font-weight:600">{{ sub.subcategory_name }}</td>
                <td data-label="Slug">{{ sub.slug }}</td>
                <td data-label="Description" class="desc-cell">{{ sub.description || '—' }}</td>
                ...
                <td data-label="Actions">
                  ...
                  <teleport to="body">
                    <transition name="fade">
                      ...
                      >
                        <button class="edit-btn" @click="editSubcategory(sub)">📝 Edit</button>
                        <button class="delete-btn" @click="deleteSubcategory(sub.id)">🗑️ Delete</button>
                      </div>
                    </transition>
                  </teleport>
                </td>
              ...
              <tr v-if="filteredSubcategories.length === 0">
                <td colspan="8" class="no-data">No subcategories found.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const editSubcategory = (subcategory) => {
  alert(`Edit: ${subcategory.subcategory_name}`);
  dropdownOpen.value = null;
}
...
@media (max-width: 850px) {
  ...
  .custom-category-table td::before { 
    ...
    text-transform: uppercase; width: 38%; flex-shrink: 0; text-align: left; padding-right: 10px; box-sizing: border-box;
  }
...
@media (max-width: 380px) {
  .card { padding: 0.7rem; }
  .custom-category-table td::before { width: 42%; }
}
